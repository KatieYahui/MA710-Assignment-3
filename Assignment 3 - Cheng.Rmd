...
The next parameter setting we want to try is to add 2-grams. Since from our previous step of base inverstigation, there are names of people and city appear as common words in the clusters. For example, "Donald" and "Trump", as well as "New" and "York".
...
The `tokenizer` is the funtion to apply n-gram words. Inside of the `NGramTokenizer` function, `weka_control(min=1, max=3)` indicates we need 1-gram, 2-gram and 3 gram words.   
...
From the frequency, the cluster 4 has the largest size, which contains 877 articles. However, cluster 1 only contains 2 articles. The clustering result is unbalanced.
...
Based on the result, we could find cluster 1 is about Fidel Castro. The common words indicate "Bat of Pigs Invasion", assassination attempts on Fidel Castro and Cuba crisis. We guess the reason he appears here is he died on November 25, 2016 and it is a big news. Right now we are not sure about the relationship of Fidel Castro and the Oscars. 

Cluster 2 is pretty interesting. It talks about the hotest news of OScars 2017- "La La land" mistakenly named best picture. It is reasonable that there is 281 articles realted to this news.

Cluster 3 is predominant by U.S. president Donald Trump. There are 50 records inside of the cluster.

Cluster 4 has common words as "film", "first", "new", "day", "night". The theme of the cluster is ambiguous and we could only guess it is related with the Oscars 2017 but not very sure.
...
We can see that the second cluster is about "La La land" mistakenly named best picture. Especially from article 5, 7 and 9. The clustering method works for this cluster. 
